...
def load_and_process_data(sample_size=None):
    ...
    try:
        # Load data from optimized CSV files
        households_df = pd.read_csv('csv_data_optimized/HHS.csv')
        persons_df = pd.read_csv('csv_data_optimized/Persons.csv')
        trips_df = pd.read_csv('csv_data_optimized/Trips.csv')
        
        # Vehicles data is not included in optimized version
        vehicles_df = None
